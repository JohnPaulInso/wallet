    ...
    window.CalendarView = {
        ...
        openDayModal: function(date, dayTxns) {
            ...
            if (dayTxns.length === 0) {
                ...
                dayTxns.forEach(t => {
                    const cat = t.manualCategory || t.category || 'Others';
                    const amt = t.manualAmount ?? t.amount ?? 0;
                    ...
                    categoryGroups[cat].txns.push(t);
                    if (isInc) totalInc += amt; else if (!t.excluded) totalExp += amt;
                });

                modalBody.innerHTML = Object.values(categoryGroups).map(group => `
        ...
        createTxnChip: function(t) {
            const amt = t.manualAmount ?? t.amount ?? 0;
            const isInc = t.manualCategory === 'Income' || t.category === 'Income';
            return `
                <div class="calendar-txn-chip ${isInc ? 'income' : 'expense'}" style="display:flex; align-items:center; gap:12px; padding:12px; background:#fff; border:1px solid #f1f5f9; border-radius:12px; margin-bottom:4px; ${t.excluded ? 'opacity:0.5' : ''}">
                    <div class="history-icon" style="width:32px; height:32px; border-radius:10px; background:#f8fafc; display:flex; align-items:center; justify-content:center; color:#64748b;">
                        <i class="material-icons" style="font-size:18px;">receipt_long</i>
                    </div>
                    <div style="flex:1;">
                        <div style="font-size:12px; font-weight:800; color:#1e293b;">${t.merchant || 'Unknown'}</div>
                        <div style="font-size:10px; color:#94a3b8; font-weight:600;">${t.manualCategory || t.category || ''}</div>
                    </div>
