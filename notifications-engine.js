...
export const NotificationsEngine = {
    ...
    getPersistentNotificationDocId(type = "general", meta = null) {
        ...
        // Goal milestones: single stable id per goal + milestone + cycle, including
        // legacy type strings like goal_<id>_100_cycle_1.
        const goalMatch = typeText.match(/^goal_(.+)_(50|75|100|1000)(?:_cycle_(\d+))?$/i);
        if (goalMatch || (safeMeta?.goalId && safeMeta?.milestone)) {
            const goalId = String(safeMeta?.goalId || goalMatch?.[1] || "");
            const milestone = String(safeMeta?.milestone || goalMatch?.[2] || "");
            const cycle = Number(safeMeta?.cycle || goalMatch?.[3] || 0);
            const baseType = `goal_${goalId}_${milestone}`;
            const base = cycle > 0 ? `${baseType}_cycle_${cycle}` : baseType;
    ...
    sameThresholdMeta(left = null, right = null) {
        ...
        if (
            left.notificationKey
            && right.notificationKey
            && String(left.notificationKey) === String(right.notificationKey)
        ) {
            return true;
        }
        if (
            ...
            && String(left.milestone || "") === String(right.milestone || "")
    ...
    async triggerNotification(uid, title, body, type = "general", meta = null) {
        ...
        const safeMeta = this.sanitizeMeta(meta);
        const persistentDocId = this.getPersistentNotificationDocId(type, safeMeta);
        let notifId = persistentDocId || `local-${createdAtMs}`;
        ...
        // Schedule before the Firestore write so realtime listeners see the same local-delivery key
        // and do not produce a second native push for this notification.
        ...
        try {
            ...
            if (!this.wasDeliveredLocally(uid, type, notifId, safeMeta)) {
                await this.deliverLocalNotification(uid, notifId, safeTitle, safeBody, type, createdAtMs, safeMeta);
            }
            return notifId;
        }
        ...
        try {
            ...
            for (const goalDoc of goalsSnap.docs) {
                ...
                if (pct >= 100 && completedCycle > 0) {
                    ...
                    const notificationKey = `${type}_cycle_${completedCycle}`;
                    const meta = {
                ...
                if (pct >= 50 && halfwayCycle > 0) {
                    ...
                    const notificationKey = `${type}_cycle_${halfwayCycle}`;
                    const meta = {
                        action: "open_goal_edit",
                        goalId,
                        source: "goals",
    ...
    async hasNotified(uid, type, meta = null) {
        try {
            ...
            const safeMeta = this.sanitizeMeta(meta);
            if (safeMeta?.goalId && safeMeta?.milestone) {
                const baseType = `goal_${safeMeta.goalId}_${safeMeta.milestone}`;
                const cycle = Number(safeMeta.cycle || 0);
                const legacyType = cycle > 0 ? `${baseType}_cycle_${cycle}` : baseType;
                const legacyDocId = `notif_persist_${this.sanitizeNotificationDocToken(legacyType)}`.slice(0, 140);
                if (legacyDocId !== persistentDocId) {
                    const legacyDoc = await getDoc(doc(db, `users/${uid}/${NOTIFICATIONS_COLLECTION}`, legacyDocId));
                    if (legacyDoc.exists()) return true;
                }

                const typeCandidates = [...new Set([type, baseType, legacyType].filter(Boolean).map(String))];
